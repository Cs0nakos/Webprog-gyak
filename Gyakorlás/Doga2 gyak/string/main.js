...
function task1(word) {
    ...
    let result = false;
    const arr = ['cs', 'dz', 'ny'] 
    arr.forEach(w => result = result || word.includes(w))

    for (let w of ['cs', 'dz', 'ny']) {
        if (word.includes(w)) {
            result = true
            break
        }
...
function task6(password) {
    ...
    // - van benne nagybetű
    // - van benne számjegy
    const r1 = new RegExp('[a-z]', 'g')
    const r2 = /[A-Z]/g
    const r3 = /\d/g
    return password.match(r1) && password.match(r2) && password.match(r3)
}
